-- Migration 042: monto_recibido/cambio en ventas (pago en efectivo)
-- Permite registrar cuanto entrego el cliente y el cambio calculado,
-- para mostrarlo despues en el recibo/historial de la venta.
...
-- =============================================
-- complete_sale() — add p_monto_recibido param
-- =============================================

DROP FUNCTION IF EXISTS public.complete_sale(UUID, UUID, UUID, public.metodo_pago, JSONB, BOOLEAN, TEXT);
...
BEGIN
  ...
  RETURN public._crear_venta_desde_items(
    p_tenant_id, v_caller_id, p_cliente_id, p_metodo_pago, p_items, p_include_iva, p_notas, p_monto_recibido
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.complete_sale(UUID, UUID, UUID, public.metodo_pago, JSONB, BOOLEAN, TEXT, DECIMAL) TO authenticated;

-- =============================================
-- _crear_venta_desde_items() — add p_monto_recibido param
-- =============================================

DROP FUNCTION IF EXISTS public._crear_venta_desde_items(UUID, UUID, UUID, public.metodo_pago, JSONB, BOOLEAN, TEXT);
...
REVOKE ALL ON FUNCTION public._crear_venta_desde_items(UUID, UUID, UUID, public.metodo_pago, JSONB, BOOLEAN, TEXT, DECIMAL) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._crear_venta_desde_items(UUID, UUID, UUID, public.metodo_pago, JSONB, BOOLEAN, TEXT, DECIMAL) TO service_role;
