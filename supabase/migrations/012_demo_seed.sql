-- =============================================
-- 012: Self-Serve Demo - tenant 'Abarrotes Don Pedro'
-- ---------------------------------------------
-- Crea una funcion idempotente public.reset_demo_tenant() que:
--   1. Garantiza la existencia del tenant demo (subdominio fijo).
--   2. Garantiza la existencia del user demo@symvora.com.
--   3. Garantiza la membresia demo user <-> demo tenant (ORG_ADMIN).
--   4. TRUNCATE en cascada de las tablas de negocio del tenant demo.
--   5. Re-inserta el snapshot: ~20 productos de abarrotes, ~10 clientes,
--      ~30 ventas del ultimo mes con su detalle, 1 caja abierta, suscripcion
--      ACTIVE, configuracion por defecto y legal_acceptances del demo user.
--   6. Devuelve el tenant_id y el user_id demo.
--
-- Usado por la API route /api/demo/start antes de generar el magic link.
-- Idempotente: se puede llamar N veces sin acumular filas (advisory lock).
-- =============================================
...
BEGIN
  -- Advisory lock para serializar resets concurrentes (e.g. dos visitantes
  -- haciendo click en "Ver demo" al mismo tiempo).
  PERFORM pg_advisory_xact_lock(hashtext('reset_demo_tenant'));

  ----------------------------------------------------------------------
  -- 1. Demo user (en auth.users via supabase admin) - aqui solo leemos.
  --    El user debe existir antes de llamar a esta funcion; ver migracion
  --    013_demo_user_seed.sql.
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 2. Tenant demo (idempotente: si existe, lo reutilizamos).
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 3. Limpieza: TRUNCATE todo lo del tenant demo.
  --    ON DELETE CASCADE en la mayoria lo haria, pero TRUNCATE es
  --    mas rapido y explicito.
  ----------------------------------------------------------------------
  TRUNCATE public.detalle_ventas RESTART IDENTITY CASCADE;
  TRUNCATE public.movimientos_caja RESTART IDENTITY CASCADE;
  ...
  -- legal_acceptances es opcional: la tabla puede no existir si la migracion
  -- 010 no se ha aplicado (e.g. en BD que no tienen modulo legal habilitado).
  BEGIN
    DELETE FROM public.legal_acceptances WHERE user_id = v_demo_user_id;
  EXCEPTION WHEN undefined_table THEN
    NULL;
  END;
  -- trial_codes usados por el demo user
  UPDATE public.trial_codes SET used_by_user_id = NULL WHERE used_by_user_id = v_demo_user_id;

  ----------------------------------------------------------------------
  -- 4. Membresia demo user <-> demo tenant (idempotente).
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 5. Tenant settings (configuracion por defecto).
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 6. Suscripcion ACTIVE (para que el middleware no redirija a billing).
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 7. Productos (snapshot abarrotes mexicanos tipicos).
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 9. Caja abierta hoy con movimientos de las ventas historicas.
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 10. Ventas historicas (ultimos 30 dias).
  --     Generamos ~30 ventas con 1-3 productos cada una.
  ----------------------------------------------------------------------
  FOR i IN 1..30 LOOP
    ...
    -- Items: 1-3 productos aleatorios
    ...
    -- Calculamos subtotal sumando los items
    v_subtotal := 0;
    v_total := 0;
    ...
    -- Actualizar totales de caja (acumulado)
    UPDATE public.cajas
  ...
  ----------------------------------------------------------------------
  -- 11. Legal acceptances (evita banner de actualizacion legal).
  ----------------------------------------------------------------------
  BEGIN
...
-- Grant execute a service_role (solo backend, no anon/authenticated)
-- El frontend nunca llama esto directo: la API /api/demo/start usa service role.
-- No concedemos a anon ni authenticated.
