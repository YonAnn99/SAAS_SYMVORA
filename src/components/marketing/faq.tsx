...
function AccordionRow({ index, question, answer, isOpen, onToggle }: AccordionRowProps) {
  return (
    <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden hover:border-blue-200 transition-colors">
      <button
        type="button"
        onClick={onToggle}
        ...
        className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
      >
        <span className="flex items-center gap-3">
          <span
            className={`w-7 h-7 shrink-0 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
              isOpen ? "bg-blue-600 text-white" : "bg-neutral-100 text-neutral-500"
            }`}
            aria-hidden="true"
          >
            {index + 1}
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="px-5 pb-4 pl-[60px] text-sm sm:text-base text-neutral-600 leading-relaxed">
              {answer}
            </div>
          </motion.div>
        )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col gap-12">
        ...
        >
          ...
          <motion.a
            ...
            className="inline-flex items-center gap-2 bg-emerald-500 text-white font-medium px-5 py-2.5 rounded-lg hover:bg-emerald-600 transition-colors shadow-sm hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.97 }}
          >
