...
├── supabase/migrations/        # 001-048 (schema, RBAC, onboarding, sales, legal, demo guards, conekta methods, referidos, códigos promo, hardening, auditoría, invite keys, IVA toggle, monto recibido, sugerencias, billing_period, security hardening — 034/035 reconstruidas el 2026-09-08, ver sesión abajo)
...
- **Conekta producción**: claves productivas configuradas en Vercel, webhook fail-closed verificado (401 sin firma), flujo de pago en efectivo probado end-to-end en producción (2026-08-31, ver Completado). Webhook de producción **ya configurado** (2026-09-08: URL correcta `app.symvora.com.mx/api/conekta/webhook`, todos los eventos seleccionados, llave de firma de producción en Vercel). **Pendiente real**: quedó una suscripción de producción creada con el plan viejo (`symvora-basic-yearly`, sin el fix de `trial_period_days`) — revisar/cancelar esa suscripción específica en el dashboard de Conekta antes de considerar el cobro anual en producción verificado end-to-end con una suscripción limpia (creada ya con `symvora-basic-yearly-v2`/`symvora-basic-monthly-v2`).
