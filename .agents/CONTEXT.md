...
└── src/
    ...
    │   └── use-is-demo.ts             # Hook cliente que detecta sesión demo combinando sessionStorage + ?demo=1 + email + app_metadata (#31)
    ...
    │   │   ├── auth.ts                # requireTenantAccess(): autentica por cookie y valida membresía/permiso por tenant (incluye isDemo: boolean en resultado desde migración 022)
    │   │   ├── demo-guard.ts          # Helpers isDemoUser() / isDemoUserSync() / assertNotDemo() para bloquear al demo user en endpoints sensibles (#31)
...
| `/[locale]/demo` | Pública | Self-Serve Demo: client component que llama `/api/demo/start` (reset + magic link para `demo@symvora.com`) y redirige al dashboard con sesión real. NO requiere login previo — está en `isPublicRoute` del middleware. Banner `DemoBanner` aparece en el dashboard cuando `?demo=1` |
...
| `/api/demo/start` | API | Iniciar demo: rate limit 5 req/min/IP → `reset_demo_tenant()` RPC → `supabase.auth.admin.generateLink()` para `demo@symvora.com`. Acepta `?locale=es|en` (fallback `Accept-Language`) |

---

## Cómo funciona la Autenticación

1. **Signup**: Formulario con acordeones (3 secciones: Datos personales, Empresa, Seguridad). Logo en panel oscuro. Sección Empresa incluye nombre, tipo negocio y logo. Sección Seguridad incluye email, contraseña con checklist, confirmar contraseña. -> `supabase.auth.signUp()` -> upload logo -> `complete_onboarding` RPC (creates tenant) -> insert subscription (trial) -> call `/api/conekta/create-checkout` -> redirect to Conekta. Fallback a `/es/billing`
...
6. **Protección de rutas**: Middleware verifica autenticación + subscription status. Usuarios con trial expirado o pago vencido son redirigidos a `/billing`. `/billing` está en `isPublicRoute` para evitar loops. **`/demo` está en `isPublicRoute`** para que visitantes no autenticados puedan ejecutar el flujo de magic link. Páginas legales (`aviso-privacidad`, `terminos`, `politica-cookies`) públicas (`isLegalRoute`)
...
- Signup: Formulario con acordeones (Datos personales, Empresa, Seguridad), logo en panel oscuro, email sin confirmación
...
- **Demo pública sin login (fix #29)**: `isPublicRoute` del middleware ahora incluye `/demo`; `/api/demo/start` y `/api/auth/callback` resuelven locale dinámicamente (`?locale=` + `Accept-Language`); `useLocale()` aplicado a los hrefs del botón "Ver demo" y del CTA del DemoBanner. El visitante llega al dashboard demo con sesión real de `demo@symvora.com` y banner visible
- **Aislamiento total del modo demo (sin tocar el sistema real)**: cierre del último vector de fuga entre el demo público y la base de datos de producción. El usuario demo (`demo@symvora.com`) es `ORG_ADMIN` del tenant demo pero hasta ahora podía disparar efectos externos si navegaba manualmente a páginas sensibles. Ahora:
  - **Helper central**: nuevo `src/lib/supabase/demo-guard.ts` exporta `DEMO_USER_EMAIL`, `isDemoUser()`, `isDemoUserSync(email)`, `assertNotDemo()`. El último devuelve `{ ok: false, response: NextResponse.json({ code: "DEMO_MODE_RESTRICTED", error: "..." }, { status: 403 }) }` cuando detecta al demo user (chequea email **y** `app_metadata.is_demo`).
  - **`requireTenantAccess` ampliado**: el resultado ahora incluye `isDemo: boolean` (chequea email + `app_metadata.is_demo`), retrocompatible con call sites existentes.
  - **12 endpoints blindados** con `assertNotDemo()` justo después de `requireTenantAccess`: `conekta/create-checkout`, `conekta/cancel-subscription`, `mercadopago/create-order`, `mercadopago/cancel-order`, `mercadopago/test-connection`, `mercadopago/config`, `facturas/stamp`, `facturas/cancel`, `facturas/create`, `facturas/config`, `users/invite`, `trial-codes/redeem`. Cada uno retornaba `403` al intentar crear customers/órdenes/CFDI reales.
  - **Migración `022_demo_guards.sql`** (aplicada en Supabase vía MCP): crea `public.is_demo_user()` (SECURITY DEFINER, solo `service_role`) y `public.current_user_is_demo()` (STABLE, ejecutable por `authenticated` para policies). Marca `auth.users.raw_app_meta_data.is_demo = true` para el demo user (Supabase sincroniza este campo con `app_metadata` que ve el backend). Refuerza `REVOKE EXECUTE ON FUNCTION public.reset_demo_tenant() FROM PUBLIC, anon, authenticated` (ya existía en migración 018, se reaplica idempotentemente para defenderse contra migraciones futuras que recreen la función).
  - **UI bloqueada**: nuevo `useIsDemo()` hook cliente (`src/hooks/use-is-demo.ts`) que combina `sessionStorage["demo_active"]`, `?demo=1` y `user.email`/`app_metadata.is_demo` (la última fuente es la definitiva). Nuevo `<DemoRestrictedNotice />` (`src/components/demo/demo-restricted-notice.tsx`) con variantes `banner` (reemplaza la página) e `inline` (dentro de un card), con copy i18n en `landing.demo.restricted.{eyebrow,title,description,cta,back}` aplicado a `/es` y `/en`. Páginas con short-circuit en sesión demo: `/(dashboard)/[locale]/billing`, `/(dashboard)/[locale]/settings/payments`, `/(dashboard)/[locale]/facturas`, `/(dashboard)/[locale]/facturas/config`. Páginas con `<DemoRestrictedNotice />` arriba + botones deshabilitados: `/(dashboard)/[locale]/users` (botón "Agregar usuario" + invitación), `/(dashboard)/[locale]/facturas` (Stamp/Cancel chequean `isDemo` antes del fetch).
  - **Tests**: nuevo `src/__tests__/demo-guard.test.ts` cubre `isDemoUser` (5 escenarios), `isDemoUserSync` (3) y `assertNotDemo` (2) — 10 tests nuevos. Suite total: 104 tests pasan. Nuevo `e2e/demo-isolation.spec.ts` (Playwright) verifica end-to-end que los 5 endpoints principales devuelven `403 + code: DEMO_MODE_RESTRICTED` tras el flujo de magic link del demo, y que las 3 páginas restringidas muestran el banner.
  - **Verificación en BD**: `reset_demo_tenant()` solo ejecutable por `postgres` + `service_role` (nunca `authenticated`/`anon`); `is_demo: true` confirmado en `auth.users.raw_app_meta_data`; las 3 funciones (`is_demo_user`, `current_user_is_demo`, `reset_demo_tenant`) existen.
  - **Documentación**: nuevo `docs/demo-isolation.md` con principio general, tabla de endpoints bloqueados, descripción de helpers, esquema de `requireTenantAccess`, hooks/componentes UI, sección de reseeding del tenant demo, tests, riesgos no cubiertos y checklist para añadir nuevos endpoints sensibles.
  - **Build/lint/types**: `npm run build` compila limpio, `tsc --noEmit` sin errores, ESLint sin nuevos warnings/errors en archivos modificados. Las reglas preexistentes de `react-hooks/set-state-in-effect` se manejan con bloques `/* eslint-disable */` siguiendo el patrón ya usado en `use-tutorial.ts` y `marketing/navbar.tsx`.
- **Accordion del signup roto al expandir + sección Personalización eliminada + Turnstile reposicionado**: fix del layout del panel de registro. Problema: al abrir un `AccordionItem` (p. ej. "Datos personales"), el panel "se rompía" y desaparecía la flecha para colapsar. Causa raíz: `.auth-sign-up form` heredaba de `.auth-container form` `height: 100%` + `overflow: hidden` + `justify-content: center`, y además `.auth-sign-up form` forzaba `overflow: hidden`. Con `justify-content: center` el desborde se reparte arriba/abajo cortando la parte superior (headers con flechas) fuera de la vista, y el `form` con `height: 100%` hacía que `.auth-form-scroll` (el scroll real) no tuviera nada que scrollear. Solución en `src/styles/auth-toggle.css`: `.auth-sign-up form { height: auto; min-height: 100%; justify-content: flex-start; overflow: visible }`. Además se **eliminó por completo** el `AccordionItem` "Personalización" (índice 3) de `auth-forms.tsx` (quedan 3 secciones: Datos personales, Empresa, Seguridad) y las claves i18n `auth.customization` + `auth.customizationHint` de `es.json`/`en.json`. El widget Cloudflare Turnstile del signup (`cf-turnstile-signup`) se reposicionó de "después del acordeón" a **justo encima del botón "Crear cuenta"**, centrado con `display:flex; justify-content:center`, para asociarlo al CTA y que no se pierda al expandir el acordeón. Login sin cambios. Verificado: `tsc --noEmit`, `npm run build`, 104 tests vitest, ESLint sin errores nuevos.
- **Fondo animado del login (`GradientWaves`) + toggle de visibilidad en password**: el fondo del layout del auth (login/signup) pasó de ser un gradiente lineal estático blanco→negro a una capa WebGL animada con olas monócromas (raymarching shader sobre `ogl`). Nueva dependencia: `ogl@^1.0.11`. Nuevo componente `src/components/auth/gradient-waves.tsx` (`"use client"`): wrapper tipado del shader, con `useReducedMotion` (motion/react) que salta WebGL cuando el usuario prefiere movimiento reducido, cleanup robusto para React 19 StrictMode (`try/catch` en `removeChild`, `cancelAnimationFrame`, `WEBGL_lose_context?.loseContext()`), pause/resume vía `IntersectionObserver` + `visibilitychange` para no gastar GPU cuando la pestaña/elemento no es visible, `pointer-events: none` por defecto (decorativo), `mouseInteraction={false}` por defecto (no distrae al escribir). Defaults calibrados para blanco/negro: `horizonColor="#FFFFFF"`, `waveColor="#1a1a1a"`, `crestColor="#A3A3A3"` (mismos hex que el gradiente CSS original), `speed={0.3}`, `amplitude={2.0}`, `tilt={1.05}`, `fogDepth={18}`, `detail="low"` (40 steps para mobile), `opacity={0.85}`. El gradiente CSS original (`linear-gradient(135deg, #ffffff → #e5e5e5 → #a3a3a3 → #1a1a1a)`) se mantiene **detrás** como fallback — primer-paint FOUC, navegadores sin WebGL2, prefers-reduced-motion. z-index layering en `src/app/(auth)/[locale]/layout.tsx`: `auth-gradient-waves` z=0, `auth-noise-overlay` z=1 (textura SVG noise), contenido z=2 (card de auth). Toggle de visibilidad en password del login: el componente `src/components/ui/password-input.tsx` ya tenía Eye/EyeOff pero solo se usaba en signup. Ahora el input pelado de `auth-forms.tsx:666-672` se reemplazó por `<PasswordInput className="auth-password-input" />`. El botón del toggle es `tabIndex={-1}` para no robar foco del input, y los estilos `.auth-password-input { width: 100%; margin: 6px 0; }` se añadieron a `auth-toggle.css` para alinear con el resto de inputs del login. Documentación en `docs/login-background.md`. **Verificación**: `npm run build` compila limpio, `tsc --noEmit` 0 errores, ESLint 0 issues en archivos modificados, 104 tests vitest pasan.
...
- **Solución**: Click en "Ver demo" navega a `/<locale>/demo`, un client component que llama `POST /api/demo/start`. Esa API: (1) ejecuta `public.reset_demo_tenant()` (RPC idempotente con `pg_advisory_xact_lock` para serializar resets concurrentes); (2) genera un magic link para `demo@symvora.com` vía `supabase.auth.admin.generateLink({ redirectTo: <origin>/api/auth/callback?next=/<locale>/dashboard?demo=1 })`; (3) devuelve el `action_link` y el frontend redirige. Supabase verifica el token → callback intercambia `code` por sesión → redirige a `/<locale>/dashboard?demo=1`. Rate limit en memoria: 5 req/min/IP.
...
- **DemoBanner**: client component que lee `?demo=1` de `useSearchParams()` (o `sessionStorage.getItem("demo_active") === "1"` como respaldo) y renderiza un banner sticky `bg-gradient-to-r from-blue-600 to-purple-600` arriba del DashboardShell con texto "Estás probando la demo. Los datos se reinician al salir." + botones "Crear mi cuenta gratis" (→ `/<locale>/signup`) y "Salir" (signOut + redirect a `/`). i18n en `landing.demo.banner.{text,cta,exit}`.
...
### 29. Banner del demo lee ?demo=1 + sessionStorage como respaldo
- **Archivos**: `src/components/demo/demo-banner.tsx`, `src/app/[locale]/demo/page.tsx`
- **Problema**: Si la cadena de redirects de Supabase perdiera el query `?demo=1` antes de llegar al dashboard, el `DemoBanner` no se mostraría y el usuario pensaría que la demo no funcionó
- **Solución**: el banner lee `useSearchParams().get("demo")` **y** `sessionStorage.getItem("demo_active") === "1"`. La página `/<locale>/demo` setea el flag en `sessionStorage` justo antes de hacer `window.location.href = action_link`. Si el query se pierde, el banner sigue apareciendo. Try/catch alrededor de `sessionStorage` para modo incógnito restrictivo

### 30. Demo bloqueada por middleware (botón "Ver demo" enviaba a /es/login)
- **Archivos**: `src/lib/supabase/middleware.ts`, `src/app/api/demo/start/route.ts`, `src/app/api/auth/callback/route.ts`, `src/app/[locale]/demo/page.tsx`, `src/components/marketing/hero.tsx`, `src/components/demo/demo-banner.tsx`
- **Problema**: Después de la implementación de la Self-Serve Demo (#28), el botón "Ver demo" del hero llevaba a `/es/login` en lugar del dashboard demo. Causa raíz: `src/lib/supabase/middleware.ts:54-58` redirige toda ruta no listada en `isAuthRoute` o `isPublicRoute` a `/es/login` si no hay sesión. `/es/demo` caía en esa zona gris — el middleware redirigía antes de que la página cliente ejecutara `fetch("/api/demo/start")` y obtuviera el magic link.
- **Solución**:
  - **`middleware.ts`**: agregado `request.nextUrl.pathname.includes("/demo")` a `isPublicRoute` (cubre `/es/demo`, `/en/demo` y `/demo` directo). Esta es la corrección mínima — el resto del flujo ya funcionaba
  - **`/api/demo/start`**: añadida función `resolveLocale(request)` que acepta `?locale=` (enviado por la página cliente) y cae a `Accept-Language`. Reemplaza el `locale = "es"` hardcoded para que el magic link lleve al dashboard con el idioma correcto
  - **`/[locale]/demo/page.tsx`**: el `fetch` ahora envía `?locale=${encodeURIComponent(resolvedLocale)}`. Se extrajo `locale` a una variable local dentro del effect para satisfacer `tsc` (`locale` es `string | null` por estado)
  - **`/api/auth/callback`**: añadida `resolveDefaultLocale(request)` que infiere locale de `Accept-Language`. El default de `next` y el redirect de error (`/es/login?error=auth`) ahora respetan el locale cuando falta `next`
  - **`hero.tsx`**: el `href` del Link usa `useLocale()` (`/${locale}/demo`) en lugar de `/es/demo` hardcoded
  - **`demo-banner.tsx`**: el CTA "Crear mi cuenta gratis" usa `useLocale()` (`/${locale}/signup`) en lugar de `/es/signup` hardcoded
- **Verificación de la cadena de redirects**: con `URLSearchParams.set('next', '/es/dashboard?demo=1')`, el `toString()` produce `next=%2Fes%2Fdashboard%3Fdemo%3D1` (correctamente URL-encoded). Cuando Supabase agrega `code` como searchParam adicional, `next` queda intacto en el destino del callback. La sesión del demo user es real (no bypass), y el `?demo=1` sobrevive la cadena de redirects; si se perdiera, `sessionStorage.setItem("demo_active", "1")` (puesto por la página `/demo`) activa el banner de todos modos
- **Verificación final**: `tsc --noEmit` 0 errores. ESLint en los 6 archivos modificados: 0 errores, 0 warnings (los 21 errores preexistentes están todos en archivos no tocados: `auth-forms.tsx`, `header.tsx`, `use-current-tenant.ts`).

### 31. CRITICAL: Demo user podía tocar integraciones externas reales
- **Archivos**: `src/lib/supabase/demo-guard.ts` (nuevo), `src/lib/supabase/auth.ts` (extendido), `src/hooks/use-is-demo.ts` (nuevo), `src/components/demo/demo-restricted-notice.tsx` (nuevo), `supabase/migrations/022_demo_guards.sql` (nuevo, aplicado en vivo), `src/__tests__/demo-guard.test.ts` (nuevo, 10 tests), `e2e/demo-isolation.spec.ts` (nuevo), `docs/demo-isolation.md` (nuevo), `src/app/api/conekta/{create-checkout,cancel-subscription}/route.ts`, `src/app/api/mercadopago/{create-order,cancel-order,test-connection,config}/route.ts`, `src/app/api/facturas/{stamp,cancel,create,config}/route.ts`, `src/app/api/users/invite/route.ts`, `src/app/api/trial-codes/redeem/route.ts`, `src/app/(dashboard)/[locale]/{billing,settings/payments,users,facturas,facturas/config,facturas/[id]}/page.tsx`, `src/messages/{es,en}.json`
- **Problema**: La Self-Serve Demo (#28) usaba la misma base de datos Supabase que producción y autenticaba al visitante como `demo@symvora.com` con rol `ORG_ADMIN` sobre el tenant demo `abarrotes-don-pedro`. RLS impedía ver/modificar filas de otros tenants, pero el demo user sí podía navegar manualmente a `/billing` y disparar `POST /api/conekta/create-checkout` → crear un **customer real en Conekta + orden de MX$400 cargable**. Mismo vector para `mercadopago/create-order` (orden POS real), `mercadopago/test-connection` (cambiar la terminal real a modo PDV), `facturas/stamp` (timbrar CFDI real en PAC), `users/invite` (enviar email real de invitación). 12 endpoints sin guard. RLS no cubre el riesgo porque el efecto externo es en un sistema de terceros, no en una fila de BD.
- **Solución**: Tres capas defensivas:
  1. **Backend**: `assertNotDemo()` invocado al inicio de cada endpoint sensible tras `requireTenantAccess`. Devuelve `403 { code: "DEMO_MODE_RESTRICTED" }`. Centralizado en `src/lib/supabase/demo-guard.ts` para que añadir nuevos endpoints sea trivial.
  2. **DB**: migración `022_demo_guards.sql` añade `public.is_demo_user()` (SECURITY DEFINER, solo `service_role`) y `public.current_user_is_demo()` (STABLE, usable en policies). Marca `auth.users.raw_app_meta_data.is_demo = true` (Supabase sincroniza con `app_metadata`). Reaplica `REVOKE EXECUTE ON FUNCTION public.reset_demo_tenant() FROM PUBLIC, anon, authenticated`.
  3. **UI**: hook `useIsDemo()` (combina sessionStorage + query param + email + app_metadata) y componente `<DemoRestrictedNotice />` (variantes `banner` e `inline`) con copy i18n. Páginas críticas cortan en seco al detectar demo; las que tienen flujos mixtos (facturas) muestran banner arriba + bloquean handlers de Stamp/Cancel.
- **Tabla de endpoints bloqueados**: `conekta/{create-checkout,cancel-subscription}` (MX$400 checkout, cancel subscription); `mercadopago/{create-order,cancel-order,test-connection,config}` (POS real, modo PDV, rotación de tokens); `facturas/{stamp,cancel,create,config}` (timbrado CFDI real vía PAC); `users/invite` (email real); `trial-codes/redeem` (canje de trial code real).
- **Acciones que SÍ quedan permitidas para el demo user**: ventas (POS) y devoluciones (vía `complete_sale` RPC), ajustes de inventario, CRUD de productos/clientes/proveedores/variantes/lotes, apertura/cierre de caja, reportes, aceptación de legales, escritura en `activity_logs` del propio tenant demo. Todo se resetea en el siguiente "Ver demo" (cada 5 min/IP como mucho).
- **Verificación**: `npm run build` compila limpio (33 endpoints en output), `tsc --noEmit` 0 errores, ESLint sin nuevos issues, **104 tests vitest pasan** (94 existentes + 10 nuevos del demo-guard), 3 verificaciones SQL directas confirman que `reset_demo_tenant` solo es ejecutable por `service_role`, `is_demo_user`/`current_user_is_demo` existen, y `raw_app_meta_data.is_demo = true` quedó fijado para `demo@symvora.com`.

### 32. Login sin toggle para ver la contraseña + fondo estático plano
- **Archivos**: `src/components/ui/password-input.tsx` (existía sin usar en login), `src/components/auth/auth-forms.tsx`, `src/components/auth/gradient-waves.tsx` (nuevo), `src/app/(auth)/[locale]/layout.tsx`, `src/styles/auth-toggle.css`, `docs/login-background.md` (nuevo), `package.json` (`ogl@^1.0.11`)
- **Problema A (UX/seguridad)**: el input de password del login (`auth-forms.tsx:666-672`) era un `<input type="password">` pelado sin toggle de visibilidad. Los usuarios no podían verificar que estaban escribiendo la contraseña correcta (especialmente en passwords largos con símbolos), y tampoco podían copiar la contraseña para pegarla en un gestor. El componente `PasswordInput` con toggle Eye/EyeOff ya existía en `src/components/ui/password-input.tsx` desde antes, pero solo se usaba en el signup (con `showChecklist`).
- **Problema B (visual)**: el fondo del auth era un `linear-gradient(135deg, #ffffff → #e5e5e5 → #a3a3a3 → #1a1a1a)` completamente estático. La card blanca encima destacaba bien pero el fondo daba sensación plana/de marketing barato — sin movimiento que sugiriera "producto vivo".
- **Solución A**: reemplazar el input pelado del login por `<PasswordInput className="auth-password-input" />`. El toggle Eye/EyeOff ya estaba implementado (líneas 52-63 de `password-input.tsx`): botón absoluto a la derecha con `tabIndex={-1}` para no robar foco. Estilos nuevos en `auth-toggle.css`: `.auth-password-input { width: 100%; margin: 6px 0; }` y `.auth-password-input > .relative { width: 100%; }` para alinear con el `margin: 6px 0` del resto de inputs del login (el selector `.auth-container input` ya estiliza el `<input>` interno: bg `#f0f0f0`, border-radius 8px, padding `11px 14px`, focus `#e0e0e0`).
- **Solución B**: nueva capa WebGL animada con olas monocromas.
  - **Dependencia**: `ogl@^1.0.11` (~30KB minified).
  - **Componente** `src/components/auth/gradient-waves.tsx` (`"use client"`): wrapper TS estricto del shader de raymarching, con adaptaciones del snippet original:
    - Uniforms tipados como `Float32Array`, retornos `tuple [number, number, number]`, props como `interface GradientWavesProps`
    - `useReducedMotion()` de `motion/react` salta WebGL si `prefers-reduced-motion: reduce` (WCAG 2.3.3) — el gradiente CSS queda detrás como fallback
    - Cleanup robusto para React 19 StrictMode: `try/catch` en `container.removeChild(canvas)` (StrictMode puede invocar cleanup antes del attach), `cancelAnimationFrame(raf)`, `gl.getExtension('WEBGL_lose_context')?.loseContext()`
    - Pause/Resume: `IntersectionObserver` pausa cuando el contenedor no es visible, `visibilitychange` pausa cuando la pestaña está oculta
    - `pointer-events: none` por defecto (decorativo puro)
    - `mouseInteraction={false}` por defecto — un fondo que reacciona al cursor distrae al usuario mientras escribe y hace que la card "tiemble" visualmente
    - WeakMap para GC automático del contexto WebGL cuando el contenedor se desmonta
  - **Defaults blanco/negro**: `horizonColor="#FFFFFF"`, `waveColor="#1a1a1a"` (mismo hex que `auth-toggle-panel` línea 214), `crestColor="#A3A3A3"` (mismo gris del stop 60% del gradiente original). Velocidad `0.3`, amplitud `2.0`, `tilt={1.05}`, `fogDepth={18}`, `detail="low"` (40 steps para mobile), `opacity={0.85}`, `grainIntensity={0.03}`.
  - **Layout**: `<GradientWaves className="auth-gradient-waves" />` se monta como primer hijo de `.auth-page-wrapper` antes del `auth-noise-overlay`. z-index layering: WebGL z=0, noise overlay z=1, contenido (card) z=2. El gradiente CSS original se mantiene **detrás** (`.auth-page-wrapper { background: linear-gradient(...) }`) — primer-paint FOUC, navegadores sin WebGL2, prefers-reduced-motion.
  - **Accesibilidad**: `role="presentation"` + `aria-hidden="true"` en el contenedor (decorativo puro, no anuncia nada al lector de pantalla).
- **Verificación**: `npm run build` compila limpio, `tsc --noEmit` 0 errores, ESLint 0 issues en archivos nuevos/modificados (`gradient-waves.tsx`, `layout.tsx`), 104 tests vitest pasan (no se añadieron tests del componente porque depende del DOM/WebGL — verificación visual manual). En DevTools → Rendering → "Emulate prefers-reduced-motion: reduce" el WebGL se desmonta y solo queda el gradiente CSS.
- **Defaults ajustados para que las olas se vean sobre blanco** (commit siguiente): el problema era que `horizonColor=#FFFFFF` + `crestColor=#A3A3A3` (gris claro) hacía que las crestas se perdieran contra el horizonte blanco en la mitad superior del viewport — el efecto quedaba plano en la zona clara. Cambios: `horizonColor=#FFFFFF` → `#E5E5E5` (gris muy claro, mismo stop 30% del gradiente CSS, evita que las crestas blancas se "fusionen" con un horizonte blanco puro), `crestColor=#A3A3A3` → `#FFFFFF` (crestas blancas que ahora contrastan sobre el gris claro), `waveColor=#1a1a1a` → `#0a0a0a` (cuerpo de las olas más oscuro para más contraste), `amplitude=2.0` → `2.8` (crestas más altas), `waveScale=0.5` → `0.7` (más detalle), `tilt=1.05` → `0.9` (horizonte más bajo = más área ocupada por olas), `fogDepth=18` → `12` (menos niebla = crestas más visibles y nítidas), `opacity=0.85` → `1.0` (WebGL opaco controla el fondo completo, el gradiente CSS queda como fallback puro detrás sin verse cuando WebGL está activo). Documentación actualizada en `docs/login-background.md`.

### 33. Personalización/paleta del signup era dead-end + olas invisibles sobre blanco
- **Archivos**: `src/components/auth/auth-forms.tsx`, `src/components/auth/gradient-waves.tsx`, `src/lib/validations/schemas.ts`, `src/messages/{es,en}.json`, `docs/login-background.md`
- **Problema A (UI muerta)**: el `AccordionItem` "Personalización" del signup (`auth-forms.tsx:544-552`) contenía un `<ColorPicker>` con 8 paletas (Coral, Azul Océano, Verde Bosque, Lavanda, Ámbar, Rosa, Turquesa, Carbón). El state `colorPrimario` se persistía en `tenants.color_primario` vía `complete_onboarding(p_color_primario)` (migración 008) y en `auth.users.raw_user_meta_data.color_primario` (signUp data). **Pero la app nunca leía este campo para aplicar tema** — la sección "Color primario" de `/settings` era puramente decorativa (swatches hardcodeados sin `onClick`, sin estado, sin aplicar CSS vars). Resultado: dead-end confirmado — el usuario invertía tiempo eligiendo un color que nunca afectaba la UI.
- **Solución A**: reemplazado el `<ColorPicker>` por un párrafo informativo corto que apunta a la sección real: "Personaliza tu cuenta desde Configuración una vez que inicies sesión" (clave `auth.customizationHint` en es.json/en.json). El `AccordionItem` se mantiene como sección informativa (índice 3) para preservar la estructura del wizard de 4 pasos. Eliminados: import de `ColorPicker`, state `colorPrimario`/`setColorPrimario`, `color_primario` del payload Zod `signupSchema`, `color_primario` del `signUp` metadata, `p_color_primario` de la llamada al RPC `complete_onboarding`, campo `color_primario` del schema Zod (`schemas.ts:35`), traducciones `auth.colorPalette` en es.json:81 y en.json:81. **NO** se elimina la columna `tenants.color_primario` de BD (requeriría migración 023 destructiva y los tenants existentes la tienen poblada — `reset_demo_tenant` también la inserta).
- **Problema B (olas invisibles sobre blanco)**: con los defaults anteriores (`horizonColor=#FFFFFF`, `crestColor=#A3A3A3`), el efecto se percibía bien en la mitad inferior pero la mitad superior del viewport quedaba plana. Causa raíz: el shader hace `mix(uHorizonColor, body, t)` con `t≈0` (lejos) muestra `uHorizonColor` y `t≈1` muestra `body = mix(uWaveColor, uCrestColor, ...)`. Como `uCrestColor` era gris claro similar al horizonte blanco, no había contraste en la transición.
- **Solución B** (ajuste de defaults en `gradient-waves.tsx`): `horizonColor=#FFFFFF` → `#E5E5E5` (gris muy claro, mismo stop 30% del gradiente CSS original), `crestColor=#A3A3A3` → `#FFFFFF` (crestas blancas que ahora contrastan sobre el gris claro), `waveColor=#1a1a1a` → `#0a0a0a` (cuerpo de las olas más oscuro), `amplitude=2.0` → `2.8`, `waveScale=0.5` → `0.7`, `tilt=1.05` → `0.9` (horizonte más bajo = más área visible de olas), `fogDepth=18` → `12` (menos niebla = crestas más nítidas), `opacity=0.85` → `1.0` (WebGL opaco controla el fondo completo; el gradiente CSS queda como fallback puro detrás sin verse cuando WebGL está activo).
- **Verificación**: `tsc --noEmit` 0 errores, ESLint 0 issues en archivos modificados (4 warnings preexistentes de `auth-forms.tsx` por `handleLogoSelect` y `<img>`, sin relación), `npm run build` compila limpio, 104 tests vitest pasan, grep `colorPrimario|colorPalette` retorna 0 resultados en `src/`.
