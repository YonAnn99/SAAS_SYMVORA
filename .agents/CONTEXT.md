...
├── supabase/migrations/        # 001-060 (schema, RBAC, onboarding, sales, legal, demo guards, conekta methods, referidos, códigos promo, hardening, auditoría, invite keys, IVA toggle, monto recibido, sugerencias, billing_period, security hardening, precio $399, RBAC billing/miembros, grants del hook de JWT, rate limit durable, contexto de middleware — 034/035 reconstruidas el 2026-09-08, ver sesión abajo)
...
- **Tablas (36, todas con RLS habilitado)**: verificado contra producción el 2026-09-14. Núcleo: tenants, tenant_settings, user_roles, role_permissions, tenant_memberships, productos, clientes, proveedores, ventas, detalle_ventas, compras, detalle_compras, cajas, movimientos_caja. Inventario avanzado: variantes_producto, stock_variantes, lotes, ajustes_inventario, ordenes_compra, detalle_orden_compra. Facturación: facturas, factura_detalle, facturas_folios, facturas_cancelaciones, factura_fiscal_secrets. Cobro/negocio: subscriptions, payment_history, codigos_promocionales, referidos, pagos_credito, pagos_terminal. Otras: activity_logs, legal_acceptances, user_invite_keys, sugerencias, user_permission_overrides (migración 055), rate_limits (migración 058). (`trial_codes` eliminada en migración 029 — sistema muerto, reemplazado por códigos promocionales.) `codigos_promocionales` y `rate_limits` tienen RLS **sin políticas** a propósito: solo se tocan por RPC SECURITY DEFINER o service role.
...
- **Tabla `codigos_promocionales`**: `codigo` (unique, normalizado a uppercase), `trial_days` (1-90, default 14), `activo`, `expira_en`, `usado_por_tenant_id` + `usado_en` (**un solo uso global**). RLS habilitado **sin políticas** — nadie lee la tabla vía PostgREST; acceso solo vía RPCs SECURITY DEFINER o service role.
...
```sql
-- Básico: 14 días de trial, sin expiración
...
- **Foto de perfil:** logo SYMVORA cuadrado (`public/symvora-logo-email.png`). **Tagline unificado:** "El sistema todo-en-uno para tu tienda". **CTA unificado:** "Prueba 14 días gratis" → `https://app.symvora.com.mx`.
- **Voz:** cercana, español MX, "el villano es la libreta/miedo al SAT, no el competidor".

### Descripciones por plataforma
- **Facebook (página de negocio, categoría Software/Empresa de software):** nombre `SYMVORA`; About: *"Sistema de punto de venta, inventario y facturación CFDI 4.0 para negocios en México. Sin comisiones por venta y soporte en español. Prueba 14 días gratis → app.symvora.com.mx"*; servicios: POS · Inventario · CFDI 4.0 · Finanzas · Compras; contacto `hola@symvora.com.mx` + ciudad/estado; cover con CTA; URL `facebook.com/symvora`.
...
- **Resistencia a picos de uso (2026-09-14)**: timeout en toda llamada saliente (`src/lib/http/timeout.ts` es la fuente única), `maxDuration` explícito en las 28 rutas de API, correos fuera de la ruta crítica de los webhooks con `after()`, rate limit durable en Postgres (migración 058) en lugar del `Map` en memoria que no funcionaba en serverless, y middleware de 4 consultas por navegación a 1 (migración 060). Ver la sesión 2026-09-14. **Pendiente el único punto que no es de código: subir Supabase a Pro** — el pool de PostgREST son 4 conexiones en Free.
...
- **Calidad**: 233 tests Vitest (20 archivos, incluye `product-import.test.ts`, `complete-sale.test.ts` con cobertura de `montoRecibido` y `timeout.test.ts`), Playwright E2E, CI GitHub Actions.
...
`src/lib/auth/remembered-account.ts` guarda **solo el correo** de la última cuenta
que entró en el dispositivo, para no teclearlo cada vez. Nunca la contraseña ni
ningún token: la sesión la maneja Supabase en sus cookies y esto no la toca, así
que **desmarcar el switch no cierra la sesión abierta**, solo deja de rellenar el
correo la próxima vez.

El switch existía en la pantalla desde antes pero su estado **no se leía en ningún
sitio** (`rememberMe` era estado de escritura): marcarlo o desmarcarlo no cambiaba
nada. Además `.auth-remember-me` estaba en `display: none` y solo se mostraba
dentro de la media query móvil, así que en computadora la opción no existía.
Ahora tiene estilos propios de escritorio (píldora oscura sobre la tarjeta clara;
la media query móvil los invierte al completo para la tarjeta oscura).

Detalle que importa: desmarcarlo **borra** lo guardado antes, no solo deja de
guardar. Si únicamente evitara escribir, el correo viejo seguiría apareciendo y el
switch parecería roto otra vez.
...
### Sesión 2026-09-14 — El hook del JWT nunca estuvo activo + auditoría de escalabilidad del backend

Dos bloques encadenados: primero un fallo que tenía **todas las escrituras del sistema rotas en producción**, y después la revisión de escalabilidad que pidió el usuario.

#### Bloque 1 — "no puedo abrir la caja desde Finanzas"

El síntoma era la caja, pero el fallo era general: **toda escritura protegida por `authorize()` estaba fallando**. Los logs de producción de esa noche lo muestran en `cajas`, `compras`, `proveedores` y `ordenes_compra`, todas con `403 / new row violates row-level security policy`. Lo que **sí** seguía funcionando eran las ventas (`complete_sale` es `SECURITY DEFINER` y salta RLS) y todas las lecturas (las políticas de `SELECT` no llaman a `authorize()`) — por eso pareció un problema aislado de un módulo.

Ver bug #37 para la cadena completa. En resumen: `authorize()` lee `auth.jwt() ->> 'user_role'`, el único que inyecta ese claim es `custom_access_token_hook`, y ese hook **no estaba dado de alta en Supabase Auth** — el panel de Authentication → Hooks estaba literalmente vacío. Además, aunque se hubiera activado, habría reventado: la migración 038 cambió su fuente de `user_roles` a `tenant_memberships` sin llevarse los permisos que la 001 había preparado para `user_roles`.

- **Migración `057_fix_access_token_hook_grants.sql`**: `GRANT SELECT` sobre `tenant_memberships` a `supabase_auth_admin` **más** una política RLS para ese rol. Hacen falta **las dos**: el GRANT sin la política deja el `SELECT` devolviendo cero filas (RLS activo, rol sin `BYPASSRLS`) y el hook escribiría `user_role: null` — el mismo fallo, pero en silencio y sin un error en los logs.
- **Orden de aplicación obligatorio**: primero la migración, después activar el hook. Al revés, el hook lanza `permission denied for table tenant_memberships`, Supabase Auth deja de emitir tokens y **se cae el login de todo el mundo**, no solo las escrituras.
- Hook creado en el dashboard (Postgres → `public.custom_access_token_hook`). Los statements que corre el panel al crearlo son los mismos de la 001 y son idempotentes.
- **Verificado de punta a punta**: la función devuelve el rol correcto para los 6 usuarios, y con una sesión nueva real en `app.symvora.com.mx` se abrió caja ("Caja abierta correctamente", `d565d0a7…`, fondo $100). Quedó esa caja de prueba abierta en el tenant `Pruebas SYMVORA`.
- ⚠️ El claim solo entra en **tokens nuevos**: tras activarlo hay que cerrar sesión y volver a entrar, refrescar la página no basta.
- Nota de método: el login por script **no sirve** para comprobar esto, Turnstile lo bloquea con `captcha_failed`. Hay que usar el navegador.

#### Bloque 2 — Escalabilidad del backend (objetivo: 10-50 negocios)

Aclaración que conviene fijar: **no hay microservicios ni workers**. Es un monolito Next.js en funciones serverless de Vercel + Supabase, con todas las páginas del dashboard como componentes cliente que consultan PostgREST directamente. No existe caché de servidor, ni cola, ni cron.

**Medido en producción, no estimado**: plan Supabase **Free / instancia Nano**, `max_connections` 60 pero el pool real de PostgREST es de **4 conexiones**; **534,187 peticiones a PostgREST en 40 días** (~558/h) generadas por ~2 usuarios (incluye el servidor de desarrollo, que apunta a la misma base); BD de 15 MB.

Cambios aplicados:

- **Timeouts en todas las llamadas salientes** — `src/lib/http/timeout.ts` es la fuente única (`TIMEOUTS`, `withTimeout`, `timeoutSignal`). Antes **no había ni un `AbortSignal` en todo `src/`**: Conekta, Mercado Pago, Resend y los PAC podían colgar la función hasta el tope de la plataforma, y Vercel levantaba más instancias que se colgaban igual — un proveedor lento se convertía en una caída propia. Conekta 8s (axios + agente HTTPS), Mercado Pago 8s (`AbortSignal`, cancela de verdad), Resend 5s, PAC 20s. El PAC va aparte **a propósito**: el timbrado es legítimamente lento y recortarlo cortaría timbrados válidos. Hay un test que fija esa relación.
- **`maxDuration` explícito en las 28 rutas** (15s solo-BD, 30s pagos, 60s CFDI). El criterio: superar con holgura el timeout del proveedor más lento que la ruta puede invocar, y nada más. Un tope alto en una ruta rápida solo hace que una función atascada ocupe recursos más tiempo.
- **Webhooks no bloqueantes** — los correos salieron de la ruta crítica con **`after()` de `next/server`** (Next 16; la doc de la versión confirma que vale en Route Handlers y que se rige por `maxDuration`). Antes `sendWelcomeEmailToOwner` (3 selects + `getUserById` + el envío por Resend) se esperaba **antes** del 200, así que un Resend lento hacía que Conekta no recibiera confirmación y reintentara el evento entero. Se paralelizaron con `Promise.all` las escrituras independientes de `subscription.paid` y `order.paid`.
- **El reembolso fallido ya no se pierde**: en `consumeFreeMonthCredit`, si el reembolso a Conekta falla, el cliente quedó cobrado **y** sin crédito. Antes eso solo hacía `console.error` y desaparecía con los logs. Ahora deja marca consultable: `SELECT * FROM payment_history WHERE reference = 'referido:reembolso_pendiente'`.
- **Rate limit durable (migración `058`)** — tabla `rate_limits` (RLS sin políticas) + RPC `consumir_rate_limit`, atómico en un solo `INSERT ... ON CONFLICT` porque leer y escribir en dos pasos tendría una carrera justo bajo concurrencia, que es cuando el límite importa. Sustituye el `Map` en memoria de `demo/start`, que **no limitaba nada** en serverless (un contador por instancia, borrado en cada arranque en frío). Añadido también a `users/invite` (20/h por tenant, protege la cuota de Resend) y `promo/apply` (10/h, contra fuerza bruta de códigos). Es **fail-open** a propósito: un fallo de BD no debe tumbar el endpoint. ⚠️ **Al probarlo, no usar `LATERAL` sobre `generate_series`**: todas las llamadas comparten snapshot y el contador se queda en 1, pareciendo que el límite no funciona. Hay que hacer llamadas separadas, como en producción.
- **Middleware de 4 consultas a 1 (migración `060`)** — `get_middleware_context` devuelve tenant, rol, estado de suscripción, `trial_end` y permisos efectivos de una vez. Corría en **cada navegación autenticada** (medido: 21,197 lecturas de `tenant_memberships` y 30,466 de `tenants`). El `update` de trial vencido pasó a `after()`: es contabilidad interna y no tiene por qué retrasar una redirección. **De paso cierra la incoherencia latente** del `.limit(1)` sin `order`: ahora usa `ORDER BY creado_en DESC`, igual que `custom_access_token_hook`.
- **Migración `059`** — índices para las 4 FK sin cubrir y `sugerencias_insert` reescrita con `(SELECT auth.uid())`; era la única política del esquema que reevaluaba `auth.uid()` **por fila**. Los 20 "índices sin usar" del advisor **no se tocaron**: con 15 MB ese aviso mide la falta de datos, no un índice sobrante.
- **Reportes** — tope de 5,000 ventas con aviso visible cuando se trunca (un reporte truncado en silencio se lee como si fueran las cifras completas del negocio). Al acotarlo apareció un **fallo latente**: el `.in("venta_id", ...)` mandaba todos los ids en la URL, y con unos cientos de ventas se supera el límite de longitud y el reporte entero muere con 414. Ahora va en lotes de 200.
- **Sentry** a `tracesSampleRate: 0.1` en los tres runtimes. Al 100% la cuota se agota justo durante un pico, que es cuando hace falta la visibilidad.

**Fuera de alcance a propósito** (corresponde al tramo de 50-300 negocios, no a 10-50): paginación y filtrado en SQL —hoy `fetchProducts`, `fetchPosProducts`, `fetchCustomers`, `fetchLots`, `fetchAdjustments`, `fetchVariants`, `fetchOrders` y `fetchPurchases` traen la tabla entera con `select("*")` y filtran en JS—, agregación en SQL para dashboard y reportes, cola de trabajos, y el `bulkUpdateProducts` que hace un UPDATE por fila. Umbral para retomarlo: un negocio con >2,000 productos o >50,000 ventas al año.

**Verificado**: `tsc` limpio, **233/233 tests** (nuevo `src/__tests__/timeout.test.ts`), lint sin regresión (7 errores / 274 avisos, idéntico a la línea base), barrido de auditoría RLS de la 054 con el resultado esperado exacto, y advisors de rendimiento sin las 4 FK ni el aviso de `sugerencias`.

⚠️ **Las migraciones 057-060 ya están aplicadas en producción y son compatibles hacia atrás**: la tabla y los RPC nuevos no los usaba el código desplegado, así que no hubo ventana rota. El código correspondiente seguía sin desplegar al cerrar la sesión.
...
- **Capturas de las guías de "Aprende" (2026-09-24)**: las 12 guías de `/es/aprende` (contenido en `src/features/marketing/aprende.ts`, páginas en `src/app/[locale]/aprende/`) están publicadas **sin capturas**; el diseño lo tolera (el paso se muestra solo con texto). Cada captura va en `public/aprende/{slug-de-la-guía}/` y **hay que enlazarla** en el campo `captura` del paso, con la ruta sin `public` (ej. `captura: "/aprende/caja-y-finanzas/cerrar-caja.png"`): copiar el archivo a la carpeta no basta. Estado al 2026-09-24: solo `primeros-pasos/` tiene imágenes, ninguna enlazada todavía; las otras 11 carpetas están vacías. Problemas encontrados al revisar la carpeta, **sin corregir** (no se renombró ni borró nada):
  - **Tres carpetas no coinciden con el slug de su guía**, así que sus imágenes no se encontrarían: `clientes-credito` → debe ser `clientes-y-credito`; `reportes-dashboard` → `reportes-y-dashboard`; `Punto-de-venta` → `punto-de-venta`. La mayúscula importa: Linux (local y Vercel) distingue mayúsculas en las rutas.
  - **Archivos PNG sin extensión** en `primeros-pasos/`: `crear-cuenta`, `crear-producto`, `configura-tu-negocio` y `configura-tu-cuenta-2`. Sin `.png` se sirven sin el tipo de contenido correcto. Además, `crear-procuto.png` (con errata) parece duplicado de `crear-producto.png`.
  - Recomendaciones: tomarlas en el tenant **Pruebas SYMVORA** (sin clientes ni cifras reales), de unos 1280 px de ancho, con nombres en minúsculas, sin acentos ni espacios. Slugs válidos: `primeros-pasos`, `punto-de-venta`, `productos-e-inventario`, `clientes-y-credito`, `compras-y-ordenes`, `caja-y-finanzas`, `reportes-y-dashboard`, `sucursales`, `usuarios-y-permisos`, `bitacora`, `configuracion`, `suscripcion`.
  - Mejora opcional: que cada paso detecte su captura sola por un nombre fijo (ej. `public/aprende/caja-y-finanzas/3.png` para el paso 3), para no tener que editar código al agregar imágenes.
  - Los videos siguen el mismo patrón: pegar la URL de YouTube en `videoYoutube` de la guía (acepta watch, youtu.be, shorts y embed).
- **Subir Supabase a plan Pro (2026-09-14)**: es el techo más duro del sistema y el único punto del plan de escalabilidad que quedó sin hacer. El pool de PostgREST son **4 conexiones** en Free/Nano y **ninguna optimización de código compensa eso** bajo carga concurrente. Todo lo demás de esa sesión ya está aplicado.
- **`/billing` le miente a quien se le acaba la prueba (2026-09-14)**: al caducar el trial, `middleware.ts` pone `tenants.subscription_status = 'expired'` pero **no toca `subscriptions.status`**, que se queda en `'trial'` (solo lo escriben el webhook de Conekta y `cancel-subscription`). Como `/billing` lee de `subscriptions` (`billing/page.tsx:101`), el usuario queda bloqueado fuera del sistema mientras la pantalla de cobro le dice "Prueba" y "termina en 0 días". Nadie le dice que tiene que pagar. Tampoco existe **correo de aviso de fin de prueba**: `email.ts` exporta 5 plantillas y ninguna cubre eso. Detectado con dos cuentas reales (`Papelería "Levi"` y `Ucs`) que caducaban al día siguiente.
- **`landing.footer.cookiesPolicy` no existe en `en.json` (2026-09-14)**: `footer.tsx:48` la renderiza sin condición y el footer sale en todas las páginas, así que en `/en` la columna Legal muestra el literal `landing.footer.cookiesPolicy`. Confirmado contra el bundle servido en producción. Las otras 41 claves que faltan en inglés son del namespace `tutorial`, que ya se sabía que no está traducido.
- **`public/sw.js` se linta aunque es generado (2026-09-14)**: está en `.gitignore` pero no en `globalIgnores` de `eslint.config.mjs`, así que 1 de los 7 errores de lint es ruido de un artefacto de build.
...
35. **La PWA no cargaba nada en modo avión: el arranque en frío nunca encontraba la página** (2026-09-11): reportado tras probar el teléfono en modo avión. `src/app/manifest.ts` declara `start_url: "/es/dashboard"`, pero a las rutas del panel **solo se llega por navegación de cliente** (`router.push` tras el login), así que el navegador nunca pide su HTML y el service worker nunca lo guarda. `defaultCache` de `@serwist/next` tampoco ayudaba: su regla de páginas filtra por `request.headers.get("Content-Type")?.includes("text/html")`, una cabecera que **las peticiones de navegación no llevan** (llevan `Accept`), así que las navegaciones caían en el cubo genérico `others` — `NetworkFirst`, **24 h de caducidad** y 32 huecos compartidos con todo lo demás. Sin entrada en caché, el fallback servía `/offline.html`, que además era un **callejón sin salida**: el Punto de Venta funciona entero en el cliente (catálogo en `localStorage`, ventas en IndexedDB) pero no había forma de llegar a él. **Reproducido en local** levantando el build de producción y matando el servidor: navegar a `/es/pos` daba exactamente la pantalla "Sin conexión" de la captura del usuario. Fix en tres piezas: (a) regla propia en `sw.ts` para las rutas offline, con caché `symvora-app-pages`, **30 días**, huecos que nadie más usa y `networkTimeoutSeconds: 3` (con datos móviles malos `navigator.onLine` miente y la petición se cuelga hasta que expira el TCP; en una caja registradora esperar es peor que servir algo de hace un rato); (b) **precalentado explícito** — `warmOfflineRoutes()` pide el HTML de `/{locale}/dashboard` y `/{locale}/pos` con un `fetch()` normal mientras hay red, que pasa por el service worker y se guarda bajo la misma URL (verificado: una página guardada así se sirve en un arranque en frío sin red); (c) `/offline.html` ofrece "Abrir Punto de Venta", **solo si esa página está de verdad en caché** — un botón que lleva a otra pantalla de "sin conexión" es peor que no tener botón. El matcher del service worker filtra **por ruta, no por `request.mode === "navigate"`**: el precalentado no es una navegación, y filtrar por modo habría guardado lo precalentado en otra caché sin que se notara. `clearAppPagesCache()` se llama al cerrar sesión porque el HTML del dashboard lleva dentro los datos de quien lo generó: en un mostrador compartido el siguiente cajero vería sin conexión el panel del anterior. **Verificado en local con sesión real**: modo avión + arranque en frío en `/es/dashboard` carga el panel completo (sidebar, KPIs, gráficas).

36. **El precalentado offline se daba por bueno con la sesión caducada** (2026-09-11): defecto introducido y detectado en la misma sesión, al probar el arreglo del bug #35 contra una sesión real que había expirado. `warmOfflineRoutes()` comprobaba `if (!response.ok) return`, pero con la sesión caducada el servidor responde **307 hacia `/login` y `fetch` lo sigue**, así que llega un 200 impecable... con el HTML del login dentro. Consecuencias: se escribía la marca `symvora_offline_warm_at` y el dispositivo se quedaba **sin Punto de Venta y sin reintentar durante 6 h**, y en el peor caso la pantalla de login quedaba guardada bajo la URL del panel (sin conexión aparecería un formulario de acceso incapaz de validar nada — peor que la página de "sin conexión", que al menos lo explica). Fix en **dos capas independientes**: el cliente descarta la respuesta si `response.redirected` o si la URL final no coincide con la pedida (`redirected` no siempre sobrevive al paso por el service worker; la URL sí), y el service worker añade un `cacheWillUpdate` que solo guarda `status === 200 && !redirected`, lo que cubre también las navegaciones normales del usuario. Lección: **un redirect seguido es un éxito para `fetch`**; en cualquier precalentado contra rutas autenticadas hay que comprobar la URL final, no solo el código de estado.
...
37. **El hook que inyecta `user_role` en el JWT nunca estuvo activo: TODAS las escrituras del sistema fallaban** (2026-09-14): reportado como "no puedo abrir la caja desde Finanzas", pero la caja solo era lo que el usuario intentó primero. En los logs de producción de esa noche fallaban igual `cajas`, `compras`, `proveedores` y `ordenes_compra`, todas con `403 / new row violates row-level security policy`. La cadena: las ~75 políticas de escritura llaman a `authorize()` → `authorize()` decide leyendo `auth.jwt() ->> 'user_role'` → si el claim no viene devuelve `FALSE` y rechaza todo → el **único** mecanismo que inyecta ese claim es `custom_access_token_hook`. Dos fallos independientes, y cualquiera de los dos bastaba: **(a)** el hook **no estaba dado de alta** en Supabase Auth — el panel de Authentication → Hooks estaba vacío, nunca se activó; **(b)** aunque se hubiera activado, habría reventado, porque la migración 038 cambió su fuente de `user_roles` a `tenant_memberships` (con buen criterio: es la fuente de verdad) pero **no se llevó los permisos**. La migración 001 había preparado `user_roles` con `GRANT ALL ... TO supabase_auth_admin` **y** una política RLS para ese rol; `tenant_memberships` no recibió ninguna de las dos, y `supabase_auth_admin` no es superusuario ni tiene `BYPASSRLS`. Fix (migración `057_fix_access_token_hook_grants.sql`): `GRANT SELECT` + política RLS para `supabase_auth_admin` sobre `tenant_memberships`, y alta del hook en el dashboard. **Hacen falta las dos cosas**: el GRANT sin la política no da error, devuelve cero filas y el hook escribe `user_role: null` — el mismo fallo, en silencio y sin rastro en los logs. **Orden obligatorio**: primero los permisos, después activar el hook; al revés el hook lanza `permission denied`, Supabase Auth deja de emitir tokens y se cae el login de todos, no solo las escrituras.

    **Lecciones, y son tres.** (1) Al reapuntar una función a otra tabla, comprobar que **los permisos de la tabla vieja viajen con ella** — aquí el rol que ejecuta no es el usuario final sino `supabase_auth_admin`, así que el fallo no aparece en ninguna prueba hecha desde la aplicación. (2) **Un módulo que falla no significa que el fallo sea de ese módulo**: lo que se rompió fue transversal y se manifestó donde el usuario tocó primero; lo que enmascaró el alcance fue que las ventas seguían funcionando (`complete_sale` es `SECURITY DEFINER` y salta RLS) y las lecturas también (las políticas de `SELECT` no llaman a `authorize()`). (3) Señal de diagnóstico reutilizable: si fallan escrituras en varias tablas a la vez, **mirar el claim antes que las políticas** — decodificar el `access_token` y comprobar que `user_role` no es `null`. El login por script no sirve para esto: Turnstile lo bloquea con `captcha_failed` y hay que usar el navegador.
...
- **Eliminado trial notice box** (franja debajo del subtitle): el `motion.div` con icono de reloj que mostraba `landing.hero.trialTitle` + `landing.hero.trialDesc` ("Prueba 14 días gratis / Sin tarjeta de crédito...") fue removido. El bloque estaba vacío tras quitar los textos pero seguía renderizando el recuadro `bg-primary/5` con borde.
...
- El hero ahora contiene: Headline (título + highlight) → Subtitle → CTAs (botones "Prueba 14 días gratis" + "Ver demo") → columna derecha con PosMockup + badge "+24.5%".
