...
- **Calidad**: 121 tests Vitest (11 archivos, incluye `product-import.test.ts` y `complete-sale.test.ts` con cobertura de `montoRecibido`), Playwright E2E, CI GitHub Actions.
...
**Pendiente que quedó abierto**: activar *Leaked Password Protection* (Supabase → Authentication → Policies; el MCP de Supabase no expone configuración de Auth, solo BD/migraciones/advisors/edge functions, así que es manual o vía Management API con un personal access token). Menores sin tocar: `tenant-context.tsx` y `middleware.ts` eligen tenant con `.limit(1)` **sin `order`** mientras `custom_access_token_hook` usa `ORDER BY creado_en DESC` (inconsistente para un usuario con 2+ tenants, latente hoy); `demo/start` sigue con rate limit en un `Map` en memoria que no sobrevive a serverless.
